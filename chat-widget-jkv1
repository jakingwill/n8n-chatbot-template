...
function initN8NChatWidget() {
    ...
    // Prevent multiple initializations and check for essential config
    if (window.N8NChatWidgetInitialized) return;
    window.N8NChatWidgetInitialized = true;
    
    if (!config.webhook.url) {
        console.error("Chat widget is not configured. Please provide a 'webhook.url' in the window.ChatWidgetConfig object.");
        return;
    }
    ...
    async function startNewConversation() {
        ...
        try {
            ...
            // The brand header is part of the chat interface now, so we don't hide it separately
